...
describe("loadInputQuery", () => {
  ...
  it("should throw an error for non-existent file", async () => {
    const nonExistentPath = path.join(
      process.cwd(),
      "test/fixtures/nonexistent.graphql",
    );

    await expect(loadInputQuery(nonExistentPath)).rejects.toThrow(
      "Failed to load input query from",
    );
  });
  ...
  it("should throw an error for invalid GraphQL", async () => {
    const queryPath = path.join(
      process.cwd(),
      "test/fixtures/malformed-query.graphql",
    );

    await expect(loadInputQuery(queryPath)).rejects.toThrow(
      "Failed to load input query from",
    );
  });
});
